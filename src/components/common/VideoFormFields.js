...
const VideoFormFields = ({ formData, setFormData, tags = [], handleCreateNewTag, showPreview = true }) => {
  ...
  const createTag = (tag) => {
    if (!tags.includes(tag) && handleCreateNewTag) {
  ...
  return (
    <div className="video-form-layout">
      {showPreview && <ThumbnailPreview imageUrl={formData.imageUrl} />}
